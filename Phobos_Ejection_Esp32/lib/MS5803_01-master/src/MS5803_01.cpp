...
boolean MS_5803::initializeMS_5803(boolean Verbose) {
    ...
    for (int i = 0; i < 8; i++ ){
    	...
    	Wire1.requestFrom((int)_address, 2);
...
void MS_5803::readSensor() {
    ...
    if (TEMP < 2000) {
		// For 1 bar model
		temperature2 = ((int64_t)dT * dT) / 2147483648ULL ; // 2^31 = 2147483648
		temperature2 = (int32_t)temperature2; // recast as signed 32bit integer
...
unsigned long MS_5803::MS_5803_ADC(char commandADC) {
	...
	Wire1.beginTransmission(_address);
    Wire1.write(CMD_ADC_CONV + commandADC);
    Wire1.endTransmission();
...
float MS_5803::readAltitude(float seaLevel) {
